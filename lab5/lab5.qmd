...
- **Result Interpretation: **The P-value is 0.02.
  ...
  - For $\alpha=0.10$, we fail to reject the null hypothesis ($0.02>0.10$). The number of students that believe in climate change is independent of whether they are from New York or Kentucky.
...
**Answer: ** Say we do t tests at $\alpha=0.05$, multiple t tests will lead to a higher chance of committing at least one Type I error ($1-0.95^{N_{times}}$). While the Tukey's HSD has taken this into consideration, making it a better test instead of multiple t tests.
...
par(mfrow = c(1, 2))
boxplot(uptake ~ Type, data = CO2, las = 1, main = 'Uptake of Different Types')
boxplot(uptake ~ Treatment, data = CO2, las = 1, main = 'Uptake of Different Treatments')
```
...
* **Result Interpretation**
  ...
  * --> Both factors have a significant effect on CO2 uptake.
...
* **Hypotheses**
  * Difference in factor `binary`
    ...
    * $H_A:$ There is a difference in the amount of money earned by each movie domestically whether they passed the Bechdel test, controlling for the decade in which the movie was made (`decade_code`).
...
* **Result Interpretation**
  ...
  * --> Both factors have a significant effect on the amount of money earned by each movie domestically.
...
| share_unemp_seas          | Percentage of the unemployed population, Sep, 2016        | numeric | Independent   |
...
my_hatecrime <- hate_crimes %>%
  select(state_abbrev, median_house_inc, share_unemp_seas, hate_crimes_per_100k_splc) %>%  # select only the interested cols
  na.omit() %>%  # remove records with NAs
  mutate(income.n=getNormalized(median_house_inc)) %>%  # get normalized income
  mutate(unemp.n=getNormalized(share_unemp_seas)) %>%  # get normalized unemployment
  mutate(income.level=cut(income.n, breaks=c(-Inf, 1/3, 2/3, Inf), labels=levels)) %>%  # get categorical income levels
  mutate(unemp.level=cut(unemp.n, breaks=c(-Inf, 1/3, 2/3, Inf), labels=levels)) %>%  # get categorical unemployment levels
  select(state_abbrev, income.level, unemp.level, hate_crimes_per_100k_splc)  # select only the interested cols
head(my_hatecrime, 5)

# Statistical Analysis
hatecrime.aov <- aov(hate_crimes_per_100k_splc~income.level+unemp.level, data=my_hatecrime)
