...
**Answer:** The `right_join()` result retains all observations in y (i.e. `trait` in this case). Therefore, some species only exist in `trait` but not in the `RY.m` remains in the result. The `inner_join()` only keeps the observations both in x and y. And the `left_join()` keeps all observations in x.
